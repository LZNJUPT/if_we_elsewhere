...
DEFAULT_RULES = {
    ...
    "断联/无互动": {"dims": {"closeness": -0.250, "trust": -0.120, "comm_quality": -0.300,
                ...
                "synthetic": True},
    # 扩展事件（二期 §5.8 · Phase C）：边界重申——拒绝立场下对方再次情感推进后的划界回应。
    # synthetic: True（真实语料无对应标注事件，按既有先例「断联/无互动」的合成规则处理）；
    # 推导方式：方向参照「冲突/分歧」的边界触碰语义、量级取其 0.4 倍（重申边界比冲突温和，
    # 但明确排斥推进），conflict 小幅上升（触碰被指出）。与私有树同名同源，勿单边改动。
    "边界重申": {"dims": {"conflict": 0.032, "closeness": -0.020, "emotional_safety": -0.020,
                         "comm_quality": -0.014},
                "reason": ("对方再次情感推进后的边界重申：明确排斥推进（亲密/情绪安全/沟通质量小幅回落），"
                           "触碰被指出使冲突指数小幅上升。合成事件，量级=冲突/分歧×0.4。"),
                "synthetic": True},
}
